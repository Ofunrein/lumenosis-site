...
export function CreativePricing({
  tag = "Simple Pricing",
  title = "Two ways in.",
  description = "Both paths lead to the same outcome: appointments on your calendar.",
  tiers,
}: {
  tag?: string;
  title?: string;
          ...
          return (
            ...
            >
              ...
              <div className="relative p-6">
                ...
                <div className="mb-5">
                  <div className={cn("w-11 h-11 rounded-full mb-4 flex items-center justify-center border-2 border-[var(--color-line)] text-[var(--color-brand-violet)]")}>
                    {tier.icon}
                  </div>
                  <h3 className="font-[family-name:var(--font-display)] text-xl text-[var(--color-ink-charcoal)]">{tier.name}</h3>
                  <p className="text-sm text-[var(--color-muted)] mt-1">{tier.description}</p>
                </div>
                <div className="mb-5">
                  <span className="text-3xl font-bold text-[var(--color-ink-charcoal)]">{tier.price}</span>
                  {tier.period && <span className="text-[var(--color-muted)] text-sm">{tier.period}</span>}
                </div>
                <ul className="space-y-2 mb-6">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-sm text-[var(--color-ink-charcoal)]">
